...
export async function POST(req: NextRequest) {
  try {
    ...
    // Save transaction record — wrapped in try/catch to never break the success response
    try {
      await (prisma as any).transaction.create({
        data: {
          ...
          fiatAmount: Number(order.price) * Number(amount),
      ...
      console.error('[Transaction] Failed to save transaction record (non-fatal):', txErr);
    }
